...
@section('title', 'addProjectMember')
...
<header>
    ...
    <h4>Add Users to Project: {{$projects->name}}</h4>
...
<body>
    ...
    <div class="project-list">
        <div class="project-list-container">
            ...
            <tbody>
                @foreach ($allUsers->except(Auth::id()) as $user)
                @if(!($user->is_admin))
                <tr>
                    <h5>{{ $user->name }}  </h5>
                    ...
                    @if (in_array($user->id, $usersInProject))
                    &#x2705;
                    <form method="POST" action="{{ route('removeUser') }}" class="removeUserForm">
                        @csrf
                        @method('DELETE')
                    ...
                    @else
                    <form method="POST" action="{{url('insert')}}" class="addUserForm">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ $user->id }}">
                        <input type="hidden" name="project_id" value="{{ $projects->id }}">
